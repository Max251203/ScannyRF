...
export default function UserModal({ open, onClose, initialUser=null, onSaved }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isEdit) {
        updatedUser = await AuthAPI.authed(`/admin/users/${initialUser.id}/`, { method: 'PUT', body: fd })
      } else {
        updatedUser = await AuthAPI.authed(`/admin/users/`, { method: 'POST', body: fd })
      }

      // Если админ отредактировал СЕБЯ — сразу обновляем локальное состояние и уведомляем приложение
      if (isEdit && updatedUser && updatedUser.id) {
        const me = JSON.parse(localStorage.getItem('user') || 'null')
        if (me && Number(me.id) === Number(updatedUser.id)) {
