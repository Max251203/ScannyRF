...
export default function Profile(){
  ...
  // Слушаем глобальные обновления пользователя (например, при редактировании себя через модалку админа)
  useEffect(() => {
...
function InfoSection({user,onUpdated}){
  ...
  const sendCode=async()=>{ try{
    const targetEmail = (email || user?.email || '').trim()
    if(!targetEmail){ toast('Сначала укажите e‑mail','error'); return }
    await AuthAPI.requestCode(targetEmail); toast('Код отправлен на почту','success')
  }catch(e){ toast(e.message,'error') } }
