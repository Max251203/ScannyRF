...
function PeniCalculator(){
  ...
  useEffect(() => {
    fetch('https://www.cbr-xml-daily.ru/key-rate.json')
      .then(r => r.ok ? r.json() : null)
      .then(j => {
        if (j && j.keyRate) {
          setKeyRate(Number(j.keyRate))
          setRateDate(j.date || '')
        }
      })
      .catch(()=>{})
  ...
  const calc = () => {
    ...
    const start = addDays(parseYmd(due), 1) // со дня, следующего за сроком уплаты
    const end   = parseYmd(pay)             // по день уплаты включительно
    if (end < start) { setRows(null); setTotal({days:0, peni:0}); return }
          ...
          <div className="calc-table">
            <table>
              ...
              <tbody>
                ...
                <tr>
                    <td>Итого</td>
                    <td className="t-num">{total?.days ?? 0}</td>
                    <td></td>
                    <td className="t-num">{fmt(total?.peni ?? 0)}</td>
                    <td></td>
                </tr>
                </tfoot>
            </table>
